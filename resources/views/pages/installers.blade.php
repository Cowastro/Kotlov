    ...
    <section class="flat-spacing">
        <div class="container">
            <div class="text-center" style="padding: 80px 0;">
                ...
                <h1 class="h3 mb-16">Монтажники KOTLOV</h1>
                <p class="text-body-1 cl-text-2 mb-40">
                    Раздел в разработке. Скоро здесь появится каталог проверенных монтажников<br class="d-none d-md-block">
                    по отоплению, дымоходам, каминам и тепловым насосам по всей Беларуси.
                </p>
    ...
    <section class="flat-spacing">
        <div class="container">
            <div class="sect-heading type-2 text-center mb-40">
                ...
                <p class="s-desc text-body-1 cl-text-2">Специализированные монтажники под каждый тип оборудования</p>
            </div>
            <div class="position-relative flat-spacing pb-0">
                <div class="br-line fake-class top-0"></div>
                <div dir="ltr" class="swiper tf-swiper"
    ...
    <section class="flat-spacing" id="catalog">
        <div class="container">
            <div class="sect-heading type-2 text-center mb-40">
                ...
                <p class="s-desc text-body-1 cl-text-2">Выберите специалиста по региону и типу работ</p>
            </div>
            <div class="row gy-4">
                <div class="col-lg-3">
                    <form method="GET" action="{{ route('installers.index') }}" id="installer-filter-form">
                        <div class="widget-facet mb-24">
                            <div class="facet-title mb-16"><p class="text-body-1 fw-medium">Область</p></div>
                            <select name="region" class="tf-select w-100" onchange="document.getElementById('installer-filter-form').submit()">
                                <option value="">Все регионы</option>
                                @foreach($regions as $value => $label)
                                    <option value="{{ $value }}" {{ request('region') === $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="widget-facet mb-24">
                            <div class="facet-title mb-16"><p class="text-body-1 fw-medium">Специализация</p></div>
                            <select name="specialization" class="tf-select w-100" onchange="document.getElementById('installer-filter-form').submit()">
                                <option value="">Все виды работ</option>
                                @foreach($specializations as $value => $label)
                                    <option value="{{ $value }}" {{ request('specialization') === $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        @if(request()->hasAny(['region', 'city', 'specialization', 'rating']))
                        <a href="{{ route('installers.index') }}" class="tf-btn btn-outline w-100">Сбросить фильтры</a>
                        @endif
                    </form>
                </div>
                <div class="col-lg-9">
