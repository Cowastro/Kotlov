...
class AiContentEnricher
{
    ...
    public function enrich(
        ...
        $specsBlock = ! empty($attributes)
            ? "Реальные характеристики товара (используй только их):\n{$attrText}"
            : ($rawSnippet ? "Краткое описание поставщика: {$rawSnippet}" : '');

        $prompt = <<<PROMPT
Ты SEO-копирайтер для белорусского интернет-магазина kotlov.by (продажа котлов, печей, каминов, кондиционеров и климатического оборудования).
...
СТРОГИЕ ПРАВИЛА:
- Используй ТОЛЬКО характеристики, указанные выше. Не придумывай параметры, которых нет в списке.
- Не указывай конкретные числа (мощность, размеры, площадь обогрева и т.д.), если их нет выше.
- Не упоминай технологии и функции, которых нет в предоставленных характеристиках.

Структура:
- 2 абзаца <p>: первый — для кого и зачем, второй — достоинства модели на основе характеристик
- список <ul><li> с 3–5 пунктами строго из характеристик выше
