...
class SyncSakovichCommand extends Command
{
    protected $signature = 'supplier:sync-sakovich
    ...
    protected $description = 'Scrape Сакович solid fuel boilers from teplodvor.by and sync prices, cards, photos and attributes.';

    private const SUPPLIER_CODE   = 'sakovich';
    private const SYNC_KEY        = 'sakovich_teplodvor';
    private const SOURCE_URL      = 'https://www.teplodvor.by/shop/kotly/tverdotoplivnye/sakovich/';
    private const BASE_URL        = 'https://www.teplodvor.by';
    private const IMAGE_DISK_PATH = 'img/products/sakovich';
    private const CATEGORY_ID     = 54;
    private const BRAND_SLUG      = 'sakovich';
    ...
    public function handle(): int
    {
        ...
        $this->info(sprintf('Found %d products on teplodvor.by/sakovich.', count($items)));
        ...
        foreach ($items as $i => $item) {
            ...
            try {
                ...
                if ($enrichContent) {
                    $seo = $enricher->generateSeo($item['name'], 'Сакович', 'твердотопливные котлы', $merged['attributes'] ?? []);
                    if ($seo) {
    ...
    private function scrapeCatalog(int $sleepMs): array
    {
        ...
        do {
            ...
            $hasNext = str_contains($html, 'class="next_page"')
                || preg_match('/href="[^"]*sakovich\/page' . ($page + 1) . '\/"/', $html);
    ...
    private function normalizeName(string $name): string
    {
        ...
        $name = preg_replace('/\b(КОТЕЛ\s+ТВЕРДОТОПЛИВНЫЙ|ТВЕРДОТОПЛИВНЫЙ\s+КОТЕЛ|КОТЕЛ|САКОВИЧ|SAKOVICH)\b/u', '', $name);
    ...
    private function upsertProduct(array $item, ?object $product, array $images, int $brandId, $now): int
    {
        ...
        $payload = [
            ...
            'meta_keywords'     => 'Сакович, твердотопливный котел, ' . $item['name'],
    ...
    private function ensureSupplier($now): int
    {
        ...
        return (int) DB::table('suppliers')->insertGetId([
            ...
            'currency_rate' => 1,
            'contact'       => self::SOURCE_URL,
            'notes'         => 'Твердотопливные котлы Сакович. Цены с teplodvor.by (BYN).',
            'is_active'     => true,
            'created_at'    => $now,
            'updated_at'    => $now,
        ]);
    ...
    private function ensureSupplierSync($now): ?int
    {
        DB::table('supplier_syncs')->updateOrInsert(
            ...
            [
                'name'            => 'Сакович',
                'code'            => self::SUPPLIER_CODE,
                'title'           => 'Сакович: твердотопливные котлы (teplodvor.by)',
                'description'     => 'Скрапит твердотопливные котлы Сакович с teplodvor.by: цены BYN, описания, фото, характеристики.',
                'command'         => 'supplier:sync-sakovich',
    ...
    {
        $base = Str::slug($name) ?: 'sakovich-product';
        $slug = $base;
        $i    = 2;
