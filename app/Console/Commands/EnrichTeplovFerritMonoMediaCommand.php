...
/**
 * Enrich "Теплов и Сухов" Феррит Моно (430-0,5, uncoated/painted) chimney
 * products with photos and material spec, sourced from the matching product
 * pages on teplov.ru.
 *
 * The mapping file (storage/app/imports/teplov-ferrit-mono-media.json) was
 * built once by joining the approved SBG price list (sheet "Тис Феррит
 * 430-0,5") against a scrape of https://teplov.ru/catalog/dymokhodnye_sistemy/
 * sistema_tis_ferrit_mono/. Same approach as EnrichTeplovMcBlackMediaCommand
 * — see that class for the MC Black line. This command only consumes the
 * mapping; it makes no outbound HTTP calls except to download the matched
 * image.
 *
 * Usage:
 *   php artisan supplier:enrich-teplov-ferrit-mono-media                (dry-run)
 *   php artisan supplier:enrich-teplov-ferrit-mono-media --apply
 *   php artisan supplier:enrich-teplov-ferrit-mono-media --apply --sku=TS.FRT.TRB.0080.32815
 */
class EnrichTeplovFerritMonoMediaCommand extends Command
{
    protected $signature = 'supplier:enrich-teplov-ferrit-mono-media
    ...
    protected $description = 'Fill photos and material spec for Теплов и Сухов Феррит Моно products from the teplov.ru media map';

    private const MAP_FILE = 'imports/teplov-ferrit-mono-media.json';
    private const IMAGE_DIR = 'img/products/teplov-ferrit-mono';
    ...
    public function handle(): int
    {
        ...
        foreach ($map as $sku => $data) {
            ...
            $hasMaterial = collect($existingSpecs)->contains(fn ($s) => mb_strtolower((string) ($s['key'] ?? $s['name'] ?? '')) === 'материал');
