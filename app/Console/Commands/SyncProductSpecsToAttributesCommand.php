...
class SyncProductSpecsToAttributesCommand extends Command
{
    protected $signature = 'products:sync-specs-attributes
        ...
        {--cleanup-junk-attributes : Delete already synced rows that are storefront/service/review/contact junk, not product specs}
    ...
    public function handle(ProductSourceEnricher $enricher): int
    {
        ...
        if ((bool) $this->option('cleanup-junk-attributes')) {
            return $this->cleanupJunkAttributeValues($enricher, $apply, $limit, $ids, $supplierCode, $activeOnly, $notArchived);
        }
     ...
     * @param array<int, int> $ids
     */
    private function cleanupJunkAttributeValues(ProductSourceEnricher $enricher, bool $apply, int $limit, array $ids, string $supplierCode, bool $activeOnly, bool $notArchived): int
    {
        $query = Product::query()
            ->whereHas('allAttributeValues')
            ->when($ids !== [], fn ($query) => $query->whereIn('id', $ids))
        ...
        $query->chunkById(200, function ($products) use ($enricher, $apply, $limit, &$checked, &$affectedProducts, &$deletedRows, &$samples): bool {
            foreach ($products as $product) {
                ...
                $deleted = $enricher->deleteJunkAttributeValues($product, $apply);
                if ($deleted <= 0) {
                    continue;
                }
        ...
        $this->info('Checked products: ' . $checked);
        $this->info('Products with junk attributes: ' . $affectedProducts);
        ...
        if ($apply) {
            $this->info('Deleted junk attribute rows: ' . $deletedRows);
        } else {
            $this->warn('Dry run only. Run with --apply --cleanup-junk-attributes to delete junk rows.');
        }
