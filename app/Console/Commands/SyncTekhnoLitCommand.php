...
class SyncTekhnoLitCommand extends Command
{
    protected $signature = 'supplier:sync-tekhnolit
    ...
    private const IMAGE_DISK_PATH = 'img/products/tekhnolit';
    private const CATEGORY_ID     = 69;
    private const BRAND_SLUG      = 'tekhnolit';
    ...
    public function handle(): int
    {
        ...
        $this->info(sprintf('Found %d products on teplodvor.by/tekhnolit.', count($items)));
    ...
    private function scrapeCatalog(int $sleepMs): array
    {
        ...
        do {
            ...
            $hasNext = str_contains($html, 'class="next_page"') || preg_match('/href="[^"]*tekhnolit\/page' . ($page + 1) . '\/"/', $html);
    ...
    private function scrapeProduct(string $url): array
    {
        ...
        // Description: <section id="description">...</section>
        $content = null;
        ...
        // Attributes: <table class="param"><tr><td class="parametr"><span>NAME</span></td><td>VALUE</td></tr>
        $attributes = [];
        preg_match_all(
        ...
        // Large images: /userfls/shop/large/...
    ...
    {
        // 1. Already tracked in supplier_products by good_id
        if ($supplierId > 0) {
    ...
    private function normalizeName(string $name): string
    {
        ...
        $name = preg_replace('/\b(ПЕЧЬ\s+ДЛЯ\s+БАНИ|ПЕЧЬ|БАННАЯ|GFS|ГЕФЕСТ|TECHNOLITE|ТЕXНОЛИТ)\b/u', '', $name);
    ...
    private function upsertProduct(array $item, ?object $product, array $images, int $brandId, $now): int
    {
        ...
        if ($product) {
            // Never overwrite non-empty content and images with empty
    ...
    {
        $base = Str::slug($name) ?: 'tekhnolit-product';
        $slug = $base;
        $i    = 2;
