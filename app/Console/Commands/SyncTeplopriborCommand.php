...
class SyncTeplopriborCommand extends Command
{
    protected $signature = 'supplier:sync-teplopribor
    ...
    protected $description = 'Scrape Теплоприбор solid fuel boilers from teplodvor.by and sync prices, cards, photos and attributes.';
    ...
    private const IMAGE_DISK_PATH = 'img/products/teplopribor';
    private const CATEGORY_ID     = 54;
    private const BRAND_SLUG      = 'teplopribor';
    ...
    public function handle(): int
    {
        ...
        $this->info(sprintf('Found %d products on teplodvor.by/teplopribor.', count($items)));
    ...
    private function scrapeCatalog(int $sleepMs): array
    {
        ...
        do {
            ...
            $hasNext = str_contains($html, 'class="next_page"')
                || preg_match('/href="[^"]*teplopribor\/page' . ($page + 1) . '\/"/', $html);
    ...
    private function normalizeName(string $name): string
    {
        ...
        $name = preg_replace('/\b(КОТЕЛ\s+ТВЕРДОТОПЛИВНЫЙ|ТВЕРДОТОПЛИВНЫЙ\s+КОТЕЛ|КОТЕЛ|КОТЁЛ|ТЕПЛОПРИБОР|TEPLOPRIBOR)\b/u', '', $name);
    ...
    private function upsertProduct(array $item, ?object $product, array $images, int $brandId, $now): int
    {
        ...
        $payload = [
            ...
            'meta_keywords'     => 'Теплоприбор, твердотопливный котел, ' . $item['name'],
    ...
    {
        $base = Str::slug($name) ?: 'teplopribor-product';
        $slug = $base;
        $i    = 2;
